// src/components/Sections/JobsLiveJobsJobCardSection.tsx
"use client";
...
export function JobsLiveJobsJobCardSection({ job }: { job: AnyJob }) {
    ...
    // Infer basic tech tags from text content
    const techFromText = (() => {
    ...
    return (
        <article className="relative">
            {/* 2-column layout: content | image (image on right for md+) */}
            <div className="grid gap-4 md:grid-cols-[1fr,320px] lg:grid-cols-[1fr,360px]">
                ...
                <aside className="order-first md:order-none">
                    <div className="relative h-44 w-full overflow-hidden rounded-xl sm:h-56 md:h-full">
                        {job.bannerImage ? (
                            <Image
                                src={job.bannerImage}
